...
| 9: remote SwiftPM | Resolved for `0.1.0-beta.1` by publishing `pomvox/pomvox-cleanup-mlx` as a standalone package with an exact dependency on the matching core tag. Development source remains in `Runtime/MLX`; deterministic export tests prevent distribution drift. |
...
Release update: the earlier local/uncommitted descriptions above record the review state, not the published distribution. Version `0.1.0-beta.1` adds tagged core/runtime packages and CI; use the current integration prompt for installation. Other stable-production gates remain unchanged.
